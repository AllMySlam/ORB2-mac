...
* 插入关键帧 ------>  处理地图点(筛选生成的地图点)  -------->  局部 BA最小化重投影误差 调整-------->   筛选新插入的关键帧
*
...
void LocalMapping::InsertKeyFrame(KeyFrame *pKF)
{
    ...
    mlNewKeyFrames.push_back(pKF);	     // 将关键帧插入到 等待处理的关键帧列表中
...
// 步骤2：计算该关键帧特征点的Bow映射关系
    ...
    mpCurrentKeyFrame->ComputeBoW();// 帧描述子用字典单词线性表示的 向量
    ...
    for(size_t i=0; i<vpMapPointMatches.size(); i++)
    {
        ...
        if(pMP)
        {
            if(!pMP->isBad())
            {
                ...
                if(!pMP->IsInKeyFrame(mpCurrentKeyFrame))// 给定关键帧是否在该点的观测帧集合内
                {
                ...
                else // 双目追踪时插入的点 可能不在 帧上 this can only happen for new stereo points inserted by the Tracking
                {
                    // 将双目或xRGBD跟踪过程中新插入的MapPoints放入mlpRecentAddedMapPoints，等待检查
...
// 步骤4：更新关键帧间的连接x关系，Covisibility图和Essential图(tree)
...
// 步骤6：对每对匹配点 2d-2d 通过三角化生成3D点,和 Triangulate函数差不多
        ...
        {
        // 步骤6.1：取出匹配特征点
            const int &idx1 = vMatchedIndices[ikp].first; // 当前 匹配对 在当前关键帧中的索引
            const int &idx2 = vMatchedIndices[ikp].second;// 当前 匹配对 在邻接关键帧中的索引
        ...
        // 步骤6.4：三角化恢复3D点，
            ...
            if(cosParallaxRays<cosParallaxStereo && cosParallaxRays>0 && (bStereo1 || bStereo2 || cosParallaxRays<0.9998))
            {
                ...
                //
                //p = ( x,y,1) 相机系归一化坐标， D = （X， Y， Z）世界坐标系下
                //其叉乘矩阵为（由行列式的线性化，第一行*-x，第二行*-y，相加可以得到第三行，故只保留前两行即可）
                //  叉乘矩阵 = [0  -1  y;     T0
                //            1   0  -x;  *  T1  * D  ===>( y * T2 - T1 ) *D = 0
                //           -y   x  0 ]     T2           ( x * T2 - T0 ) *D = 0
                //一个点两个方程,两个点四个方程, A × D =0,求三维点D对A奇异值分解
                cv::Mat A(4,4,CV_32F);
            ...
            //   步骤6.4：对于双目，视差角较小时，二维点利用深度值反投影成三维点
...
// 步骤1：获得当前关键帧在covisibility图中权重排名前nn的一级邻接关键帧
    ...
    const vector<KeyFrame*> vpNeighKFs = mpCurrentKeyFrame->GetBestCovisibilityKeyFrames(nn);// 一级相邻
...
// 步骤2：获得当前关键帧在 其一级相邻帧的  covisibility图中权重排名前5的二级邻接关键帧
        ...
        const vector<KeyFrame*> vpSecondNeighKFs = pKFi->GetBestCovisibilityKeyFrames(5);// 遍历每一个 二级相邻帧
...
// 步骤3：将当前帧的 地图点MapPoints 分别与 其一级二级相邻帧的 地图点 MapPoints 进行融合
    ...
    for(vector<KeyFrame*>::iterator vit=vpTargetKFs.begin(), vend=vpTargetKFs.end(); vit!=vend; vit++)//与当前帧 匹配的地图点
    {
        KeyFrame* pKFi = *vit;
...
// 步骤4：将一级二级相邻帧所有的地图点MapPoints 与当前帧（的MapPoints）进行融合
    ...
    vpFuseCandidates.reserve(vpTargetKFs.size()*vpMapPointMatches.size());// 帧数量 × 当前帧地图点数量
    ...
    //一级二级相邻帧所有的地图点与当前帧融合
    // 投影地图点MapPoints到当前帧上，在附加区域搜索匹配关键点，并判断是否有重复的地图点
...
cv::Mat LocalMapping::ComputeF12(KeyFrame *&pKF1, KeyFrame *&pKF2)
{
    ...
    cv::Mat t12 = -R1w*R2w.t()*t2w+t1w;// tw2  + t1w    // c2 --->c1
 ...
 *  则认为该关键帧为冗余关键帧。
 *  注意：
 *      一个局部关键帧中：有效mapPoints个数为nMPs，满足有三个观测帧及尺度要求的mapPoint个数为nRedundantObservations，
 *      统计数量，RedundantObservations>0.9*nMPs 则该局部关键帧为冗余帧，可以删除
 * @param  pKF1 关键帧1
 * @param  pKF2 关键帧2
 * @return 两个关键帧之间的基本矩阵 F
 */
...
// 步骤1：根据Covisibility Graph 关键帧连接 图提取当前帧的 所有共视关键帧(关联帧)
    ...
    {
        KeyFrame* pKF = *vit;// 当前帧的每一个局部关联帧
        if(pKF->mnId==0)//第一帧关键帧为初始化世界关键帧 跳过
            continue;
// 步骤2：提取每个共视关键帧的 地图点 MapPoints
        const vector<MapPoint*> vpMapPoints = pKF->GetMapPointMatches();// 局部关键帧匹配的地图点
        int nObs = 3;
