...
cv::Mat PnPsolver::iterate(int nIterations, bool &bNoMore, vector<bool> &vbInliers, int &nInliers)
{
    ...
    // vAvailableIndices为每次从mvAllIndices中随机挑选mRansacMinSet=4组3D-2D对应点进行一次RANSAC
    vector<size_t> vAvailableIndices;
    int nCurrentIterations = 0;
    while(mnIterations<mRansacMaxIts || nCurrentIterations<nIterations)//mRansacMaxIts=300, nIterations=5
    {
        nCurrentIterations++;
        mnIterations++;
...
// 步骤1：第一个控制点：参与PnP计算的参考3D点的几何中心（质心）
...
// 步骤2：计算其它三个控制点，C1, C2, C3通过PCA分解得到（三个主方向）
...
double PnPsolver::compute_pose(double R[3][3], double t[3])
{
    ...
    // 步骤1：获得EPnP算法中的四个控制点（质心和三个主方向）
    choose_control_points();
    // 步骤2：计算世界坐标系下每个3D点用4个控制点线性表达时的系数alphas，公式1
    compute_barycentric_coordinates();
    // 步骤3：构造M矩阵，公式(3)(4)-->(5)(6)(7)
    CvMat * M = cvCreateMat(2 * number_of_correspondences, 12, CV_64F);//12未知参数（4个控制点*3维参考点坐标）提成列向量
    ...
    compute_rho(rho);// 计算四个控制点任意两点间的距离，总共6个距离
