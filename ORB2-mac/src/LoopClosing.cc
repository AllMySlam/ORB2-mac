...
* 9】筛选共有单词大于 minCommons  且词袋BOW匹配最低得分大于minScore      lscoreAndMatch
...
// 步骤3：遍历所有共视关键帧，计算当前关键帧与每个共视关键帧 的bow相似度得分，并得到最低得分minScore
    const vector<KeyFrame*> vpConnectedKeyFrames = mpCurrentKF->GetVectorCovisibleKeyFrames();// 当前帧的所有 共视关键帧
    const DBoW2::BowVector &CurrentBowVec = mpCurrentKF->mBowVec;// 当前帧的 BoW 字典单词描述向量
    float minScore = 1;
    for(size_t i=0; i<vpConnectedKeyFrames.size(); i++)// 遍历每一个共视关键帧
    {
        ...
        float score = mpORBVocabulary->score(CurrentBowVec, BowVec);// 匹配得分小越相似,越大差异越大 ？ TODO qzc 有疑问
...
// 步骤5：在候选帧中检测 具有连续性的 候选帧
    ...
    // 步骤5.3：  遍历之前的“子连续组”
        ...
        {
            set<KeyFrame*> sPreviousGroup = mvConsistentGroups[iG].first;// 取出一个之前的 子连续组
    ...
    // 步骤5.5 ： 连续
            if(bConsistent)
            {
                int nPreviousConsistency = mvConsistentGroups[iG].second;//与子候选组连续的之前一个子连续组序号
                int nCurrentConsistency = nPreviousConsistency + 1;//当前子连续组序号
                ...
                {
                    mvpEnoughConsistentCandidates.push_back(pCandidateKF);// TODO：闭环候选帧
...
// 步骤3：将当前帧mpCurrentKF与闭环候选关键帧pKF匹配 得到匹配点对
    ...
    //步骤3.2：  根据匹配点对 构造Sim3求解器
            ...
            pSolver->SetRansacParameters(0.99,20,300); // 至少20个内点inliers
    ...
    while(nCandidates>0 && !bMatch)
    {
        ...
        for(int i=0; i<nInitialCandidates; i++)
        {
            ...
            if(!Scm.empty())
            {
                ...
                {
                    // 保存符合sim3变换的内点inlier的地图点MapPoint
                    if(vbInliers[j])
...
// 步骤6：G2O Sim3优化，只要有一个候选帧通过Sim3的求解与优化，就跳出停止对其它候选帧的判断
                ...
                if(nInliers>=20)
                {
                    ...
                    mpMatchedKF = pKF;// 闭环帧 TODO qzc loopclosing kf
...
// 步骤8：取出闭环匹配上关键帧的相连关键帧，得到它们的MapPoints放入mvpLoopMapPoints
 ...
 * 2. 用当前帧在闭环地图点 mvpLoopMapPoints 中匹配的当前帧闭环匹配地图点 mvpCurrentMatchedPoints
 *    更新当前帧之前的匹配地图点 mpCurrentKF->GetMapPoint(i)
 * 2. 将闭环帧以及闭环帧相连的关键帧的所有地图点 mvpLoopMapPoints 和 当前帧相连的关键帧的点进行匹配
...
// 步骤1：停止全局优化
    ...
    if(isRunningGBA())
    {
        ...
        //停止全局优化
        mbStopGBA = true; // 这个标志位仅用于控制输出提示，可忽略
...
// 步骤4：通过位姿传播，得到Sim3优化后，与当前帧相连的关键帧的位姿，以及它们的MapPoints
...

    {
        ...
        for(vector<KeyFrame*>::iterator vit=mvpCurrentConnectedKFs.begin(), vend=mvpCurrentConnectedKFs.end(); vit!=vend; vit++)
        {
            ...
            if(pKFi!=mpCurrentKF)// currentKF在前面已经添加
            {
                cv::Mat Tic = Tiw*Twc;// 得到当前帧到pKFi帧的相对变换
        ...
        for(KeyFrameAndPose::iterator mit=CorrectedSim3.begin(), mend=CorrectedSim3.end(); mit!=mend; mit++)
        {
            ...
            g2o::Sim3 g2oCorrectedSiw = mit->second;//优化后的位姿：世界到帧
...
// 步骤5：检查当前帧的地图点MapPoints 与闭环检测时匹配的MapPoints是否存在冲突，对冲突的MapPoints进行替换或填补
        ...
        for(size_t i=0; i<mvpCurrentMatchedPoints.size(); i++)// 遍历每一个当前帧在闭环匹配时的地图点
        {
            if(mvpCurrentMatchedPoints[i])
            {
                MapPoint* pLoopMP = mvpCurrentMatchedPoints[i];//当前帧 在闭环检测匹配的地图点
                MapPoint* pCurMP = mpCurrentKF->GetMapPoint(i);//当前帧之前的地图点
                if(pCurMP)// 如果有重复的MapPoint（当前帧和匹配帧各有一个），则用闭环匹配得到的代替现有的
                    pCurMP->Replace(pLoopMP);
                else// 如果当前帧没有该MapPoint，则直接添加
                {
                    mpCurrentKF->AddMapPoint(pLoopMP,i);//帧添加关键点对应的地图点
                    pLoopMP->AddObservation(mpCurrentKF,i);// 地图点添加帧和其上对应的关键点id
 ...
 * @param CorrectedPosesMap  闭环相邻帧及其对应的sim3位姿
 */
void LoopClosing::SearchAndFuse(const KeyFrameAndPose &CorrectedPosesMap)
{
    ...
    for(KeyFrameAndPose::const_iterator mit=CorrectedPosesMap.begin(), mend=CorrectedPosesMap.end(); mit!=mend;mit++)
    {
        ...
        // mvpLoopMapPoints 为闭环时，相邻关键帧上的地图点
        ...
        for(int i=0; i<nLP;i++)
        {
            MapPoint* pRep = vpReplacePoints[i];//地图点被关键帧上的点代替
            if(pRep)
            {
